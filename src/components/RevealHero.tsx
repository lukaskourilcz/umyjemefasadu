...
export default function RevealHero({ before, after, label }: Props) {
  ...
  useRafScroll(() => {
    ...
    const progress = distance > 0 ? clamp((navH - rect.top) / distance, 0, 1) : 0;
      ...
      >
        {mobile && (
          <input
            ...
            aria-valuetext={`${dragProgress} % odkryté čisté fasády`}
        ...
        {/* BEFORE - dirty, sits underneath and shows wherever AFTER hasn't
            wiped in yet. */}
        <div className="absolute inset-0 -z-10" aria-hidden={hasPhotos ? undefined : true}>
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0">
          <div
            ...
            style={{
              background: "linear-gradient(0deg, rgba(16,24,32,0.55) 0%, rgba(16,24,32,0) 100%)",
            }}
          />
          <div
            className={`container-page relative flex items-end justify-between gap-4 pb-8 ${mobile ? "hidden" : ""}`}
          >
  ...
  if (show && src) {
    return (
      <img
        ...
        width={kind === "before" ? 1140 : 1134}
        height={kind === "before" ? 906 : 860}
        loading="eager"
        decoding="async"
        {...{ fetchpriority: "high" }}
